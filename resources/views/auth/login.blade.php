    ...
    <form method="POST" action="{{ route('login') }}" class="mt-8 w-full">
        ...
        <div class="flex items-center justify-between mt-1">
            ...
            @if (Route::has('password.request'))
                <a
                    class="text-sm text-gray-600 hover:text-green-500 rounded-md focus:outline-none"
        ...
        <div class="flex flex-col items-end mt-6">
            ...
            <a
                ...
                href="{{ route('register') }}"
            >
                {{ __('Não possui uma conta?') }} <span class="text-green-500">{{ __('Cadastre-se aqui.') }}</span>
            </a>
        </div>
    </form>
</x-guest-layout>